...
import time
import random
from solver_BFS import solve_bfs, valid
pygame.font.init()
...
class Grid:
    ...
    def update_gui_callback(self, board, row, col, placed):
        ...
        pygame.time.delay(10)  # Adjust the delay for better visual effects
    ...
    def solve_gui(self):
        self.update_model()  # Update the current model to the latest grid state
        if solve_bfs(self.model, callback=self.update_gui_callback):
            pygame.display.update()
            return True
        else:
            pygame.display.update()
