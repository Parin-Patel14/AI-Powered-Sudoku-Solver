...
def generate_puzzle(difficulty):
    ...
    # 'difficulty' parameter should be an integer representing the number of filled cells you want (easier puzzles have more filled cells).
...
class Grid:
    ...
    def update_gui_callback(self, board, row, col, placed):
        """ Update the GUI for a specific cell during the solving process. """
        value = board[row][col] if placed else 0
        self.cubes[row][col].set(value)
        self.cubes[row][col].draw_change(self.win, placed)
        pygame.display.update()
        pygame.time.delay(100)  # Delay to make the changes observable

    def solve_gui(self):
        self.update_model()  # Ensure the current model is up-to-date
    
        if solve(self.model, update_callback=lambda b, r, c, p: self.update_gui_callback(b, r, c, p)):
            # If the solver completes successfully
            pygame.display.update()
            return True
        else:
